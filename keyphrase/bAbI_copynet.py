...
config  = setup_bAbI()   # load settings.
for w in config:
    print '{0}={1}'.format(w, config[w])

logger  = init_logging(config['path_log'] + '/experiments.{0}.id={1}.log'.format(config['task'],tmark))
...
# idx2word_o(3) and word2idx_o(4) are from original source, size=132
# idx2word(1) and word2idx(2) are from target, replacing name/color/shape to <tag>, size=102
...
# first 20% is testing data, last 80% is training data
...
while echo < epochs:
    ...
    def output_stream(dataset, batch_size, size=1):
        '''
        Creates minibatches from data streams with padding.
        :return:
        '''
    ...
    # save the weights.
    agent.save(config['path_h5'] + '/experiments.Copy.bAbI.id={0}.epoch={1}.pkl'.format(tmark, echo))
